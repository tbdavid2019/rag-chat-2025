...
在 `demo.html` 中，我們新增了以下展示功能：

1. **API Key 認證模態框 (Modal)**
   - 移除硬編碼的 Key，改為啟動時彈出輸入框
   - 支援將 Key 儲存於 localStorage，再次訪問無需重新輸入
   - 提供「重設 Key」功能方便切換用戶

2. **智能問答交互**
   - **常見問題引導**：首頁提供預設的常見問題（如 "這知識庫可幫我什麼忙?"），點擊即問
   - **思考狀態顯示**：在 LLM 生成回應期間顯示 "Thinking..." 動畫，提升用戶體驗
   - **Markdown 渲染**：完整支援 Markdown 格式顯示（列表、粗體、程式碼區塊等），讓回答更清晰易讀

3. **視覺優化**
   - 採用 Tailwind CSS 打造現代化界面
   - 響應式佈局，支援各種螢幕尺寸
